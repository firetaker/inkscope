...
var  processMonApp=angular.module('processMonApp', ['ngRoute','ngTable','D3Directives','ui.bootstrap','dialogs'])
    .filter('bytes', funcBytesFilter)
    .config(function ($routeProvider) {
        $routeProvider.
            when('/', {controller: ListCtrl}).//, templateUrl: 'partials/pools/aboutPools.html'}).
            when('/detail/:processId', {controller: DetailCtrl, templateUrl: 'partials/detailProcessMon.html'}).
...
function refreshPools($http, $rootScope, $templateCache) {
  ...
  //  var  $to=document.getElementById('to').value;
   ...
   // console.log(datetime_to_unix($to));
    ...
    $http({method: "get",url: "../inkscopeCtrl/ceph/mon?skip="+$skip+"&limit="+$limit, cache: $templateCache}).
...
function DetailCtrl($rootScope,$scope, $http, $routeParams, $route, $dialogs) {
    ...
    var para = {"daemontype" : "mon."+$host,"timestamp":$timeFromTo};
